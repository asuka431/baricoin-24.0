# CJDNS support in Baricoin Core

It is possible to run Baricoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Baricoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Baricoin network and individual nodes.
...
before it will work with your Baricoin Core node. This is described in steps
...
## Run Baricoin Core with CJDNS

Once you are connected to the CJDNS network, the following Baricoin Core
...
When enabled, this option tells Baricoin Core that it is running in an
...
IPv6 local network. This helps Baricoin Core perform better address management:
...
CJDNS support was added to Baricoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `baricoin-cli -addrinfo` to see the
...
## CJDNS-related information in Baricoin Core

There are several ways to see your CJDNS address in Baricoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `baricoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `baricoin-cli getpeerinfo`).
