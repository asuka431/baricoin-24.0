...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BaricoinUnit>("BaricoinUnit");
#else
    qRegisterMetaType<BaricoinUnit>("BaricoinUnit");
#endif
}
...
static bool InitSettings()
{
    ...
    if (!gArgs.ReadSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("baricoin-core", "Settings file could not be read");
        std::string error_translated = QCoreApplication::translate("baricoin-core", error.c_str()).toStdString();
    ...
    if (!gArgs.WriteSettingsFile(&errors)) {
        std::string error = QT_TRANSLATE_NOOP("baricoin-core", "Settings file could not be written");
        std::string error_translated = QCoreApplication::translate("baricoin-core", error.c_str()).toStdString();
        InitError(Untranslated(strprintf("%s:\n%s\n", error, MakeUnorderedList(errors))));
...
bool BaricoinApplication::createOptionsModel(bool resetSettings)
{
...
void BaricoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    ...
    connect(window, &BaricoinGUI::quitRequested, this, &BaricoinApplication::requestShutdown);
...
void BaricoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
void BaricoinApplication::startThread()
{
    ...
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BaricoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BaricoinApplication::handleRunawayException);
    connect(this, &BaricoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BaricoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}
...
bool BaricoinApplication::event(QEvent* e)
{
