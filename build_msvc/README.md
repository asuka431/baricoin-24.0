...
Introduction
---------------------
Visual Studio 2022 is minimum required to build Baricoin Core.

Solution and project files to build with `msbuild` or Visual Studio can be found in the `build_msvc` directory.

To build Baricoin Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build Baricoin Core with the GUI, a static build of Qt is required.
...
```cmd
msbuild build_msvc\baricoin.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```
